  ...
  return <div className="change-monitor">
    ...
    <section className="change-workbench">
      <form className="change-controls panel" onSubmit={runComparison}>
        ...
        <label>Detection threshold <output>{draft.threshold}/100</output><input type="range" min="35" max="85" value={draft.threshold} onChange={(event) => setDraft((current) => ({ ...current, threshold: Number(event.target.value) }))} /><small>Higher values return fewer, stronger candidates.</small></label>
        ...
        <div className="change-date-grid"><label>Cloud estimate <output>{draft.cloudCoverPercent}%</output><input type="range" min="0" max="45" value={draft.cloudCoverPercent} onChange={(event) => setDraft((current) => ({ ...current, cloudCoverPercent: Number(event.target.value) }))} /></label><label>Registration error <output>{draft.registrationErrorM.toFixed(1)} m</output><input type="range" min="0" max="6" step="0.2" value={draft.registrationErrorM} onChange={(event) => setDraft((current) => ({ ...current, registrationErrorM: Number(event.target.value) }))} /></label></div>
