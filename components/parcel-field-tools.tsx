...
export default function ParcelFieldTools({ parcel, notify, onShare }: { parcel: Parcel; notify: (message: string) => void; onShare: () => void }) {
  ...
  return <div className="parcel-field-tools">
    ...
    <details><summary><Scissors size={15} aria-hidden />Local split estimate</summary><div className="split-tool"><label>Proposed split <output>{splitPercent}% / {100 - splitPercent}%</output><input aria-label="Proposed parcel split percentage" type="range" min="20" max="80" value={splitPercent} onChange={(event) => setSplitPercent(Number(event.target.value))} /></label><div><i style={{ width: `${splitPercent}%` }} /><i /></div><p><b>Child A {(parcel.area * splitPercent / 100).toFixed(0)} m²</b><b>Child B {(parcel.area * (100 - splitPercent) / 100).toFixed(0)} m²</b></p><small>Area estimate only. This does not create survey geometry or alter a parcel.</small></div></details>
