...
import SourceMetadataDialog from "@/components/source-metadata-dialog";
...
const titles: Record<PageKey, { eyebrow: string; title: string; subtitle: string }> = {
  ...
  satellite: { eyebrow: "Transparent monitoring mathematics", title: "Construction Change Monitor", subtitle: "Screen deterministic before/after observations, map candidate footprints and route results for officer review." },
  gnss: { eyebrow: "GPS · Galileo · GLONASS · BeiDou", title: "GNSS Sky View", subtitle: "Explore simulated satellite geometry, elevation masks, signals and dilution-of-precision quality." },
...
function getAssistantResponse(question: string): Message {
  ...
  if (q.includes("0012") || q.includes("analyse parcel")) {
    ...
    return { ...base, text: `Parcel ${parcel.upi} is a ${metric.areaM2.toLocaleString(undefined, { maximumFractionDigits: 0 })} m² synthetic ${parcel.landUse.toLowerCase()} parcel in ${parcel.sector} Sector, ${parcel.district}. Its centroid is approximately ${metric.roadDistanceM.toFixed(0)} m from KN 5 Road and ${metric.wetlandDistanceM.toFixed(0)} m from the closest demonstration wetland boundary.`, stats: [{ label: "Geodesic area", value: `${metric.areaM2.toFixed(0)} m²` }, { label: "Road distance", value: `${metric.roadDistanceM.toFixed(0)} m` }, { label: "UTM CRS", value: "EPSG:32736" }], sources: ["GeoJSON — Synthetic Parcel Geometry", "Turf.js — geodesic metrics", "Proj4js — UTM 36S centroid"], warning: "AI-generated decision support. Final administrative or legal decisions require an authorized NLA officer.", actions: [{ label: `Open parcel ${parcel.upi}`, page: "map", params: { parcel: parcel.upi } }, { label: "Run another analysis", page: "analysis" }] };
  }
  if (q.includes("construction") || q.includes("change detection") || q.includes("new building")) return { ...base, text: "Open Construction Change Monitor to compare two demonstration epochs. Its browser engine publishes the score equation, applies cloud and registration penalties, maps candidate footprint polygons and exports an officer-review queue.", stats: [{ label: "Model", value: "Weighted change" }, { label: "Data", value: "Synthetic" }, { label: "Review", value: "Required" }], sources: ["NLA GeoAI — deterministic change engine", "Copernicus Sentinel-2 — production data candidate"], warning: "The current workflow does not analyse live imagery and cannot prove that construction occurred.", actions: [{ label: "Open Change Monitor", page: "satellite" }] };
  if (q.includes("gnss") || q.includes("gps") || q.includes("galileo") || q.includes("glonass") || q.includes("beidou") || q.includes("dop")) return { ...base, text: "Open GNSS Sky View to compare GPS, Galileo, GLONASS and BeiDou geometry. The demonstration uses azimuth/elevation observations, an adjustable elevation mask and a design-matrix calculation for HDOP, VDOP, PDOP and GDOP.", stats: [{ label: "Constellations", value: "4" }, { label: "DOP", value: "Computed" }, { label: "Epoch", value: "Simulated" }], sources: ["GPS.gov — GPS space segment", "EUSPA — Galileo", "BeiDou Navigation Satellite System — official overview"], warning: "Sky positions are deterministic simulation, not live ephemeris or GeoNet receiver data.", actions: [{ label: "Open GNSS Sky View", page: "gnss" }, { label: "Open CORS stations", page: "cors" }] };
  ...
  if (requestedMapTopics.length && !/\b(document|law|policy|manual|report|publication)\b/.test(q)) {
    ...
    return { ...base, text: `I found ${matchingMaps.length} relevant online map services for ${requestedMapTopics.join(" and ")}: ${matchingMaps.map((layer) => layer.title).join(", ")}. Open Interactive Rwanda Map → Rwanda maps to add them as live overlays, combine up to four and adjust opacity.`, stats: [{ label: "Relevant maps", value: String(matchingMaps.length) }, { label: "Full catalogue", value: String(RWANDA_ONLINE_MAP_LAYERS.length) }, { label: "Provider keys", value: "None" }], sources: matchingMaps.map((layer) => `${layer.provider} — ${layer.title} — ${layer.licence}`), warning: "Public visibility does not automatically grant unrestricted reuse. Review the licence shown for each source before publication or operational use.", actions: [{ label: "Open these map layers", page: "map", params: { layers: matchingMaps.slice(0, 4).map((layer) => layer.id).join(",") } }, { label: "Browse full map library", page: "maplibrary" }] };
  }
  if (q.includes("nsdi") || q.includes("dataset") || q.includes("roads")) return { ...base, text: "Yes. The NSDI demonstration catalogue contains a National Road Network dataset maintained by RTDA. It has national coverage, a 1:10,000 reference scale, EPSG:32736 coordinates and internal access classification.", stats: [{ label: "Catalogue matches", value: "2" }, { label: "Latest update", value: "02 Aug 2026" }, { label: "Access", value: "Internal" }], sources: ["NSDI Catalogue — National Road Network", "NSDI Catalogue — Administrative Boundaries"], actions: [{ label: "Open NSDI catalogue", page: "catalogue" }] };
  if (q.includes("subdivision") || q.includes("document") || q.includes("law") || q.includes("policy") || q.includes("manual") || q.includes("report") || q.includes("publication")) {
    ...
    return {
      ...
      actions: [{ label: "Open official documents", page: "knowledge" }, { label: "Review provenance policy", page: "provenance" }],
    };
  }
...
export default function GeoAIApp() {
  ...
  function runAssistantAction(action: AssistantAction) {
    const url = new URL(window.location.href);
    url.searchParams.set("page", action.page);
    Object.entries(action.params ?? {}).forEach(([key, value]) => value ? url.searchParams.set(key, value) : url.searchParams.delete(key));
    window.history.replaceState(null, "", url);
        ...
        <div className="content-wrap">
          <AnimatePresence mode="wait" initial={false}>
            <motion.div className={`page-stage page-${page}`} key={page} initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 16, rotateX: -1.5 }} animate={{ opacity: 1, y: 0, rotateX: 0 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: -8 }} transition={{ duration: reduceMotion ? 0.08 : 0.28, ease: [0.22, 1, 0.36, 1] }}>
              ...
              {page === "assistant" && <AssistantPage onAction={runAssistantAction} notify={notify} />}
              ...
              {page === "satellite" && <ChangeDetectionPage notify={notify} />}
              {page === "gnss" && <GnssSkyViewPage notify={notify} initialObserver={gnssObserver} />}
...
function GeoAISupportBubble({ onNavigate }: { onNavigate: (page: PageKey) => void }) {
  ...
  return <div className={`support-widget ${open ? "open" : ""}`}>
    <AnimatePresence>{open && <motion.section className="support-panel" role="dialog" aria-label="NLA GeoAI support" id="geoai-support-panel" initial={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 18, scale: 0.96, rotateX: -4 }} animate={{ opacity: 1, y: 0, scale: 1, rotateX: 0 }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, y: 12, scale: 0.97 }} transition={{ duration: 0.22, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <div className="support-actions"><button onClick={() => { setOpen(false); onNavigate("map"); }}><span><MapIcon size={15} aria-hidden /></span>Open street map</button><button onClick={() => { setOpen(false); onNavigate("satellite"); }}><span><Satellite size={15} aria-hidden /></span>Monitor change</button><button onClick={() => { setOpen(false); onNavigate("gnss"); }}><span><Orbit size={15} aria-hidden /></span>Open GNSS sky</button><button onClick={() => ask("Calculate the area of parcel 1/02/03/04/0012.")}><span><LandPlot size={15} aria-hidden /></span>Check parcel 0012</button></div>
...
function AssistantPage({ onAction, notify }: { onAction: (action: AssistantAction) => void; notify: (s: string) => void }) {
  ...
  return <div className="assistant-layout">
    ...
    <section className="chat-panel"><div className="chat-status"><div><span className="ai-orb">✦</span><p><b>NLA GeoAI</b><small><i /> {localAiState === "ready" ? "Local AI + grounded GIS tools" : "Grounded GIS tools · local AI optional"}</small></p></div><button onClick={() => notify("Conversation exported to audit-safe text")}>Export</button></div>
      <div className="messages">
        {messages.map((message) => <div className={`message ${message.role}`} key={message.id}>{message.role === "assistant" && <span className="message-avatar">✦</span>}<div className="message-body"><small>{message.role === "assistant" ? "GEOAI ASSISTANT" : "YOU"}</small><p>{message.text}</p>{message.stats && <div className="answer-stats">{message.stats.map((stat) => <div key={stat.label}><b>{stat.value}</b><span>{stat.label}</span></div>)}</div>}{message.sources && <div className="sources"><b>Sources used</b>{message.sources.map((source) => <span key={source}>↗ {source}</span>)}</div>}{message.warning && <div className="answer-warning"><b>Human verification required</b><span>{message.warning}</span></div>}{(message.actions?.length || message.stats) && <div className="message-actions">{message.actions?.map((action) => <button key={`${action.page}-${action.label}`} onClick={() => onAction(action)}>{action.label}</button>)}{message.stats && <button onClick={() => notify("Assessment added to a report draft")}>Add to report</button>}</div>}</div></div>)}
...
function MapPage({ notify, highlightedUpis, initialTab = "layers" }: { notify: (s: string) => void; highlightedUpis: string[]; initialTab?: MapTab }) {
  ...
  const [licenceFilter, setLicenceFilter] = useState<"all" | RwandaLicenceClass>("all");
  const [inspectingLayer, setInspectingLayer] = useState<RwandaOnlineMapLayer | null>(null);
  ...
  const filteredOnlineLayers = RWANDA_ONLINE_MAP_LAYERS.filter((layer) => {
    ...
    const matchesLicence = licenceFilter === "all" || getLayerLicenceClass(layer) === licenceFilter;
    const searchable = `${layer.title} ${layer.description} ${layer.provider} ${layer.tags.join(" ")}`.toLowerCase();
    return matchesCategory && matchesLicence && searchable.includes(catalogueQuery.trim().toLowerCase());
  ...
  return <div className={`map-workspace ${initialTab === "catalogue" ? "library-entry" : ""}`}><div className="map-main"><ParcelMap selected={selected} onSelect={setSelected} onClearSelection={() => setSelected(undefined)} onNotify={notify} visibleLayers={layers} highlightedUpis={highlightedUpis} onlineLayerIds={onlineLayerIds} onlineLayerOpacity={onlineLayerOpacity} basemap={basemap} onBasemapChange={setBasemap} onLayerStatusChange={(id, status) => setLayerStatuses((current) => current[id] === status ? current : { ...current, [id]: status })} /></div>
    <aside className="map-side"><div className="map-tabs"><button className={mapTab === "layers" ? "active" : ""} onClick={() => setMapTab("layers")}>Active <em>{onlineLayerIds.length}</em></button><button className={mapTab === "catalogue" ? "active" : ""} onClick={() => setMapTab("catalogue")}>Map library <em>{RWANDA_ONLINE_MAP_LAYERS.length}</em></button><button className={mapTab === "basemaps" ? "active" : ""} onClick={() => setMapTab("basemaps")}>Basemaps <em>{Object.keys(BASEMAPS).length}</em></button></div>
      {mapTab === "layers" ? <>
        ...
        <div className="catalogue-filter-row"><select className="catalogue-category" aria-label="Filter Rwanda map category" value={catalogueCategory} onChange={(event) => setCatalogueCategory(event.target.value as typeof catalogueCategory)}><option>All</option>{RWANDA_MAP_CATEGORIES.map((category) => <option key={category}>{category}</option>)}</select><select className="catalogue-category" aria-label="Filter Rwanda maps by licence" value={licenceFilter} onChange={(event) => setLicenceFilter(event.target.value as typeof licenceFilter)}><option value="all">All licence classes</option><option value="open">Open reuse</option><option value="attribution">Attribution required</option><option value="verify">Verify / unknown terms</option></select></div>
        ...
        <div className="basemap-grid">{(Object.keys(BASEMAPS) as BasemapKey[]).map((key) => { const item = BASEMAPS[key]; return <button key={key} className={basemap === key ? "active" : ""} onClick={() => { setBasemap(key); notify(`${item.label} basemap selected`); }}><span className={`basemap-preview ${item.tone}`}><MapIcon size={22} aria-hidden /></span><p><b>{item.label}</b><small>{item.detail}</small><em className="basemap-licence">{item.licence}</em></p><i>{basemap === key ? "Active" : "Use map"}</i></button>; })}</div><div className="basemap-guidance"><ShieldCheck size={16} aria-hidden /><p><b>Choose for the task</b><small>Use Light or Dark for overlays, Topographic for terrain, Humanitarian for settlement context, Satellite to check boundaries against real imagery and Daily satellite for recent national conditions.</small></p></div>
      </div>}
    </aside>{inspectingLayer && <SourceMetadataDialog layer={inspectingLayer} onClose={() => setInspectingLayer(null)} />}</div>;
}
