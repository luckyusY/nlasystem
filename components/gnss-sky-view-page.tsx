...
import { CONSTELLATION_META, generateGnssEpoch, GNSS_OBSERVERS, type GnssConstellation, type ObserverKey } from "@/lib/gnss-engine";
...
export default function GnssSkyViewPage({ notify, initialObserver = "kigali" }: { notify: (message: string) => void; initialObserver?: ObserverKey }) {
  ...
  const epoch = useMemo(() => generateGnssEpoch({ observer: observerKey, epochMinutes, elevationMask: mask, selectedConstellations: selected }), [epochMinutes, mask, observerKey, selected]);
  ...
  return <div className="gnss-workspace">
    ...
    <section className="gnss-layout">
      <aside className="gnss-controls panel">
        ...
        <label>Elevation mask <output>{mask}°</output><input type="range" min="0" max="40" step="5" value={mask} onChange={(event) => setMask(Number(event.target.value))} /><small>Satellites below this horizon angle are excluded.</small></label>
        <label>Epoch <output>{timeLabel}</output><input type="range" min="0" max="1435" step="5" value={epochMinutes} onChange={(event) => setEpochMinutes(Number(event.target.value))} /></label>
